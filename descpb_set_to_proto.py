...
import os
import sys
from google.protobuf.descriptor_pb2 import DescriptorProto, FieldDescriptorProto, FileDescriptorProto, FileDescriptorSet
...
def main():
    if len(sys.argv) != 3:
        print('Protobuf File Descriptor Set Converter')
        print(f'Usage: {sys.argv[0]} <file name> <output folder>')
        sys.exit(1)

    filename = sys.argv[1]
    if not os.path.isfile(filename):
        print(f'Cannot find {filename}, please check your path.')
        sys.exit(1)
    ...
    for i in fd.file:
        filename, content = descpb_to_proto(i)
